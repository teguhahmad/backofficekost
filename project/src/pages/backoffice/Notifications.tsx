...
interface Notification {
  ...
  target_group?: 'all' | 'property_managers' | 'property_seekers';
  created_at: string;
}
...
const Notifications = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    target_group: 'all' as 'all' | 'property_managers' | 'property_seekers'
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error: insertError } = await supabase
        ...
        .insert([{
          ...formData,
          target_user_id: formData.target_group === 'all' ? null : formData.target_user_id,
  ...
  const getTargetGroupLabel = (group?: string) => {
    switch (group) {
      ...
      case 'property_seekers':
        return 'Property Seekers';
      default:
        return 'All Users';
    }
  };
                    ...
                    <div className="mt-2 flex items-center justify-between text-sm">
                      <div className="text-gray-500">
                        ...
                        {notification.target_user_id ? (
                          ...
                          <span>
                            Target Group: {getTargetGroupLabel(notification.target_group)}
                          </span>
                        )}
                      </div>
                      <Button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => setFormData(prev => ({ 
                    ...prev, 
                    target_group: e.target.value as 'all' | 'property_managers' | 'property_seekers',
                    target_user_id: e.target.value === 'all' ? null : prev.target_user_id
                  }))}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  required
                >
                  ...
                  <option value="property_seekers">Property Seekers</option>
                </select>
              </div>
              ...
              {formData.target_group !== 'all' && (
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                  >
                    <option value="">All users in group</option>
                    {users
                      .filter(user => 
                        formData.target_group === 'property_managers' 
                          ? user.role === 'admin' 
                          : user.role !== 'admin'
                      )
                      .map(user => (
